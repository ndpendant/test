...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput("Drug_1",label = "Drug 1", choices = unique(db$Drug),selectize = FALSE, size = nrow(db)),
      selectInput("Drug_2",label = "Drug 2", choices = unique(db$Drug),selectize = FALSE,size = nrow(db))
      ),
