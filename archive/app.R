...
server <- function(input, output,session) {
  ...
  observeEvent(input$select_button1, {
    ...
    observeEvent(input$select_button3, {
      ...
      showModal(modalDialog(title=paste(modal_name),tags$div(tags$img(src = modal_view),tags$style(padding-left = "200px")),easyClose=TRUE,footer=paste("source:",modal_view))    
)
      modal_name <<- NULL
      modal_view <<- NULL
