...
server <- function(input, output,session) {
  ...
  check_me <- reactive({
   ...
   # }
    ...
    if(sum(str_detect(test$Database,"DrugBank")>0))
    {
      ...
      dbank$Structure <- shinyInput(actionLink,nrow(dbank),"dbstruct_",rownames(dbank),icon = "www/chemical.png",label = "structure",onclick = 'Shiny.onInputChange(\"select_button1\",  this.id)' )#$#HTML(readLines(paste0("https://www.drugbank.ca/structures/",dbank$DrugID,"/image.svg")))
      holddb <- NULL
    ...
    if(sum(str_detect(test$Database,"SuperCYP")>0))
    {
      ...
      sc$Structure <- shinyInput(actionButton,nrow(sc),"kstruct_",rownames(sc),icon = "www/chemical.png",label = "structure",onclick = 'Shiny.onInputChange(\"select_button2\",  this.id)' )#$#HTML(readLines(paste0("https://www.drugbank.ca/structures/",dbank$DrugID,"/image.svg")))
    ...
    if(sum(str_detect(test$Database,"KEGG")>0))
    {
      ...
      k$Structure <- shinyInput(actionButton,nrow(k),"kstruct_",rownames(k),icon = "www/chemical.png",label = "structure",onclick = 'Shiny.onInputChange(\"select_button3\",  this.id)' )#$#HTML(readLines(paste0("https://www.drugbank.ca/structures/",dbank$DrugID,"/image.svg")))
      for(i in k$Drug)
      {
        print(i)
        print(length(k$Drug))
  ...
  observeEvent(input$select_button1, {
    ...
    observeEvent(input$select_button3, {
      ...
      showModal(modalDialog(title=paste(modal_name),tags$img(src = modal_view,height="350px"),easyClose=TRUE,footer=paste("source:",modal_view))    
)
      modal_name <<- NULL
      modal_view <<- NULL
